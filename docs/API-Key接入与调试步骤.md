...
`.env` 文件已经建好，位于 `app/server/.env`。如果没有，可从模板复制：
...
浏览器打开：`http://localhost:3000/`（新版单页应用，含首页 + 对话 + 合同预览）。

> 新版前端基于 dc-runtime 设计驱动，API 桥接层 (`api-client.js`) 会在页面渲染后自动加载并连接后端。
...
> - `housing_lease` 这个标识符要和后端 `app/server/templates/` 下的文件夹名完全一致。

---

## 六、安全提醒

- 真实 Key 只写在本地 `app/server/.env`，**不要**写进任何会 commit 的文件（本仓库是公开开源仓库）。
